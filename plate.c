...
int txt(char** filenames, int numfiles) {
...
int clang(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
...
int cplusplus(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
...
int python(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
...
int go(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
...
int haskell(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
...
int perl(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
...
int scalas(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
        err = asprintf(&name, "%s", filenames[i]);
...
int node(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
        err = asprintf(&name, "%s", filenames[i]);
...
int pythons(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
        err = asprintf(&name, "%s", filenames[i]);
...
int html(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
...
int htmls(char** filenames, int numfiles) {
    char *name;
    int err;
    for (int i = 2; i < numfiles; i++) {
        err = asprintf(&name, "%s.html", filenames[i]);
    ...
    for (int i = 2; i < numfiles; i++) {
        ...
        switch ( namesum ) {
            case TXT:  // text files. Most would just use touch to make empty files but since this takes like 5 seconds to implement included it anyway
                err = txt(filenames, numfiles);
                break; 

            case CLANG: // C file case
                err = clang(filenames, numfiles);
                break;

            case CPLUSPLUS: // C++ file case
                err = cplusplus(filenames, numfiles);
                break;
            ...
            case PYTHON: // python!
                err = python(filenames, numfiles);
                break;

            case GO: // Google Go AKA golang
                err = go(filenames, numfiles);
                break;
            ...
            case HASKELL: // haskell
                err = haskell(filenames, numfiles);
                break;
            ...
            case PERL:  // Perl script
                err = perl(filenames, numfiles);
                break;

            case SCALAS: // scala script
                err = scalas(filenames, numfiles);
                break;

            case NODES: // node script
                err = node(filenames, numfiles);
                break;

            case PYTHONS: // python script
                err = pythons(filenames, numfiles);
                break;

            case HTML:  // basic html file
                err = html(filenames, numfiles);
                break;

            case HTMLS:  // html with script tag
                err = htmls(filenames, numfiles);
                break;
            ...
            case CPROJ: // this was more of an exercise in learning file operations in C than anything. It actually works.
                if (!proj_flag) {
                proj_flag = TRUE;
                ...
                if ( stat(name, &dir) == -1) {  // make sure the directory doesn't exist. stat returns -1 if it doesn't
                    ...
                    if ( fileexists( name ) || fileexists( chmfile ) ) {  printf( "The file %s or %s already exists!\n", name, chmfile );  break;}
                        ...
                        char *ctempl;
                        err = asprintf( &ctempl, "#include \"headers/%s.h\"\n\nint main(int argc, char* argv[]){\n    printf(\"%s\", \"Hello world!\\n\");\n\n    return 0;\n}\n", filenames[i], "%s" ); 
                        createfile( name, ctempl );
                        char *upper;
                        err = asprintf( &upper, "%s", filenames[i] );
                        uppercaser( upper );
                        ...
                        char *make;
                        printf("DONE\n" );
                        err = asprintf( &make, "C=gcc\nCFLAGS=-Wall -pedantic\n\nexecutables=%s\n\nbin/%s.o: %s.c\n\t$(C) $(CFLAGS) %s.c -o bin/%s\n\nrun:\n\t./bin/%s\n\nclean:\n\trm -rf bin/%s\n\n", filenames[i], filenames[i], filenames[i], filenames[i], filenames[i], filenames[i], filenames[i] );
                        printf("1\n");
                        //printf("%s", make);
                        char *makefilename;
