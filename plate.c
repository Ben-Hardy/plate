    ...
    for ( int i = 2; i < numfiles; i++ ) {
        ...
        switch ( namesum ) {
            ...
            case CPLUSPLUS: // C++ file case
                asprintf( &name, "%s.cc", filenames[i] );
                if ( fileexists( name ) ) {  printf("The file %s already exists!\n", name);  break;}
                    else {
            ...
            case HFILE: // .h file case. 
                asprintf( &name, "%s.h", filenames[i] );
                if ( fileexists( name ) ) {  printf("The file %s already exists!\n", name);  break;}
                else {
                    char *upper;
                    asprintf( &upper, "%s", filenames[i] );
                    uppercaser( upper );
                    char *templ;
            ...
            case CPPH: // C++ wih header file
                asprintf( &name, "%s.c", filenames[i] );  // for future reference, cannot start switch cases/if statements with declarations, must start with statements or else will not compile!
                char *hfile;
                asprintf( &hfile, "%s.h", filenames[i] );
            ...
            case CHMAKE: // C file with an accompanying .h file and makefile
                asprintf( &name, "%s.c", filenames[i] );
                char *chmfile;
                asprintf( &chmfile, "%s.h", filenames[i] );
            ...
            case CPPHMAKE: // C++ wih header file
                asprintf( &name, "%s.c", filenames[i] );
                char *hmfile;
                asprintf( &hmfile, "%s.h", filenames[i] );
            ...
            case JAVA: // java!
                if ( filenames[i][0] >= 97 && filenames[i][0] <= 122 ) // if the first character of the filename is a small letter, capitalize it!
                    filenames[i][0] -= 32;
                asprintf( &name, "%s.java", filenames[i] );
                if ( fileexists( name ) ) {  printf( "The file %s already exists!\n", name );  break;}
                else {
                    char *templ;
                    asprintf( &templ, "public class %s {\n\n\tpublic %s() {\n\n\t}\n\n\tpublic static void main(String[] args) {\n\n\t\tSystem.out.println(\"Hello World!\");\n\t}\n}\n", filenames[i], filenames[i] );
                    createfile( name, templ );
                    free( name );
                    name = NULL;
                    free( templ );
                ...
                break;
            case PYTHON: // python!
                asprintf( &name, "%s.py", filenames[i] );
                if ( fileexists( name ) ) {  printf( "The file %s already exists!\n", name );  break;}
                else {
            ...
            case GO: // Google Go AKA golang
                asprintf( &name, "%s.go", filenames[i] );
                if ( fileexists( name ) ) {  printf( "The file %s already exists!\n", name );  break;}
                else {
            ...
            case SCALA: // scala!
                if (filenames[i][0] >= 97 && filenames[i][0] <= 122) // if the first character of the filename is a small letter, capitalize it!
                    filenames[i][0] -= 32;
                asprintf( &name, "%s.scala", filenames[i] );
                if ( fileexists( name ) ) {  printf( "The file %s already exists!\n", name );  break;}
                else {
                    char *templ;
                    asprintf( &templ, "object %s {\n\n\tdef main(args: Array[String]) {\n\t\tprintln(\"Hello World!\")\n\t}\n}", filenames[i] );
                    createfile( name, templ );
                    free( name );
                    name = NULL;
                    free( templ );
            ...
            case HASKELL: // haskell
                asprintf( &name, "%s.hs", filenames[i] );
                if ( fileexists( name ) ) {  printf( "The file %s already exists!\n", name );  break;}
                else {
            ...
            case SH:  // basic sh script
                asprintf( &name, "%s.sh", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else {
                    char templ[] = "#!/bin/sh\n\n\n";
                    createfile( name, templ );
            ...
            case HTML:  // basic html file
                asprintf( &name, "%s.html", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else {
            ...
            case CSS: // rudementary CSS file
                asprintf( &name, "%s.css", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else {
                    char templ[] = "body {\n\n}\n"; 
                    createfile( name, templ );
            ...
            case JAVASCRIPT: // basic javascript file with a makeshift main since main isn't required in JS
                asprintf( &name, "%s.js", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                ...
                break;
            case RUBY:  // ruby
                asprintf( &name, "%s.rb", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else { 
            ...
            case PHP: // PHP for posterity's sake
                asprintf( &name, "%s.php", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else { 
                    char templ[] = "<?php\n\n?>\n"; 
                    createfile( name, templ );
            ...
            case OCAML:  // Objective-CAML
                asprintf( &name, "%s.ml", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else { 
                ...
                break;
            case ERLANG: // erlang!
                asprintf( &name, "%s.erl", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else { 
                    char templ[] = "\n"; 
                    createfile( name, templ );
            ...
            case COFFEE:  // coffeescript 
                asprintf( &name, "%s.coffee", filenames[i] );
                if ( fileexists( name ) ) printf( "The file %s already exists!\n", name );
                else { 
